...
To upgrade to v23.2, you must be running v23.1. If you are not running v23.1, first [upgrade to v23.1]({% link cockroachcloud/upgrade-to-v23.1.md %}). Then return to this page and continue to [Step 2](#step-2-select-your-cluster-size).
...
Approximately 72 hours after all nodes are running v23.1, the upgrade will be automatically finalized. This enables certain [features and performance improvements introduced in v23.2](#expect-temporary-limitations). Finalization also removes the ability to roll back to v23.1, so it's important to monitor your applications during this 72-hour window and, if you see unexpected behavior, [roll back the upgrade](#roll-back-the-upgrade) from the CockroachDB {{ site.data.products.cloud }} Console.
</section>
...
<section class="filter-content" markdown="1" data-scope="single-node">
Your single-node cluster will be unavailable for a few minutes while the node is stopped and restarted with v23.2.
</section>

## Step 6. Monitor the upgrade

Once your cluster is running v23.2, you will have approximately 72 hours before the upgrade is automatically finalized. During this time, it is important to [monitor your applications](#monitor-your-application) and [respect temporary limitations](#expect-temporary-limitations).
...
- If you see unexpected behavior, you can [roll back to v23.1](#roll-back-the-upgrade) during the 72-hour window.
...
For an expanded list of features included in the v23.2 release, see the [v23.2 release notes](https://www.cockroachlabs.com/docs/releases/v23.2).

### Roll back the upgrade

If you see unexpected behavior, you can roll back the upgrade during the 72-hour window.

To stop the upgrade and roll back to v23.1, click **Roll back** in the banner at the top of the CockroachDB {{ site.data.products.cloud }} Console, and then click **Roll back upgrade**.

<section class="filter-content" markdown="1" data-scope="multi-node">
During rollback, nodes will be reverted to v23.1 one at a time without interrupting the cluster's health and availability.
</section>

<section class="filter-content" markdown="1" data-scope="single-node">
Because your cluster contains a single node, the cluster will be briefly unavailable while the node is stopped and restarted with v23.1. Be sure to [prepare for this brief unavailability](#prepare-for-brief-unavailability) before starting the rollback.
</section>
...
After finalization, all [temporary limitations](#expect-temporary-limitations) will be lifted, and all v23.1 features are available for use. However, it will no longer be possible to roll back to v23.1. If you see unexpected behavior after the upgrade has been finalized, [contact support](https://support.cockroachlabs.com/hc/requests/new).
